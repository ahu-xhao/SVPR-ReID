...
class AGReID(BaseImageDataset):
    ...
    def __init__(self, root=r'../datasets/', verbose=True, pid_begin=0, **kwargs):
        ...
        attr_path = osp.join(self.dataset_dir, 'qut_attribute_v4_88_attributes.mat')
    ...
    def _process_dir(self, dir_path, train=False, mode='mix', gallery_pids=None):
        ...
        for img_path in sorted(img_paths):
            ...
            # assert 0 <= pid <= 599  # pid == 0 means background
            # assert 0 <= camid <= 6
...
class AGReID_AG(AGReID):
    dataset_dir = "AGReID"
    dataset_name = 'agreid_ag'
    ...
    def make_dataset(self, train_dir, query_dir, gallery_dir):
        ...
        query_temp = self._process_dir(query_dir, train=False, mode='aerial', gallery_pids=gallery_pids)
        query = [entry for entry in query_temp]
        self.logger.info(f'query: {len(query_temp)} -> {len(query)}')

        return train, query, gallery


class AGReID_GA(AGReID):
    dataset_dir = "AGReID"
    dataset_name = 'agreid_ga'
    ...
    def make_dataset(self, train_dir, query_dir, gallery_dir):
        ...
        gallery_pids = {item[1] for item in gallery}
        query_temp = self._process_dir(query_dir, train=False, mode='ground', gallery_pids=gallery_pids)
        query = [entry for entry in query_temp]
        self.logger.info(f'query: {len(query_temp)} -> {len(query)}')
...
if __name__ == '__main__':
    ...
    logger = setup_logger("transreid", save_dir=None, if_train=True)
    dataset = AGReID()
    dataset_ag = AGReID_AG()
    dataset_ga = AGReID_GA()
