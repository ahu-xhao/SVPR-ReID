...
def do_train(cfg,
    ...
    iters_per_ehoch = len(train_loader) + 1
    log_period = iters_per_ehoch // (cfg.SOLVER.LOG_PERIOD - 1)
    ...
    for epoch in range(1, epochs + 1):
        ...
        model.train()
        for n_iter, (img, vid, target_cam, target_view, target_time, text) in enumerate(train_loader):
            optimizer.zero_grad()
            ...
            with amp.autocast(enabled=True):
                ...
                if model_name in ['ViT', 'ViT_basline']:
        ...
        if cfg.MODEL.DIST_TRAIN:
            ...
            logger.info("Epoch {}(total {} iters) done. Time per batch: {:.3f}[s] Speed: {:.1f}[samples/s]"
                        .format(epoch, n_iter, time_per_batch, train_loader.batch_size / time_per_batch))
        ...
        if epoch % eval_period == 0 or epoch == epochs:
            if cfg.MODEL.DIST_TRAIN:
                ...
                model.eval()
                for n_iter, (img, vid, camid, camids, viewids, timeids, _, text) in enumerate(val_loader):
                    with torch.no_grad():
                        img = img.to(device)
                        camids = camids.to(device)
                ...
                cmc, mAP, mINP, _, _, _, _, _ = evaluator.compute(cross_time=cfg.TEST.CROSS_TIME)
...
def do_inference(cfg,
    ...
    for n_iter, (img, pid, camid, camids, viewids, timeids, imgpath, text) in enumerate(val_loader):
        with torch.no_grad():
            ...
            text = text.to(device) if text is not None else None
            feat = model(img, cam_label=camids, view_label=target_view, time_label=target_time, text=text)
            evaluator.update((feat, pid, camid, timeids))
