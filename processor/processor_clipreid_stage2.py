    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % eval_period == 0 or epoch == epochs:
            if cfg.MODEL.DIST_TRAIN:
                ...
                model.eval()
                for n_iter, (img, vid, camid, camids, target_view, target_time, _, _) in enumerate(val_loader):
                    with torch.no_grad():
                        img = img.to(device)
                        camids = camids.to(device)
                        target_view = target_view.to(device)
                        target_time = target_time.to(device)
                        feat = model(img, cam_label=camids, view_label=target_view, time_label=target_time)
                        evaluator.update((feat, vid, camid))
                cmc, mAP, _, _, _, _, _ = evaluator.compute()
                logger.info("Validation Results - Epoch: {}".format(epoch))
                logger.info("mAP: {:.2%}".format(mAP))
                ...
                logger.info(f'Best result: {best_model_info["epoch"]} {best_model_info["mAp"]:.2%} {best_model_info["rank1"]:.2%}')
                torch.cuda.empty_cache()
...
def do_inference(cfg,
    ...
    for n_iter, (img, pid, camid, camids, target_view, target_time, imgpath, _) in enumerate(val_loader):
        with torch.no_grad():
            img = img.to(device)
            camids = camids.to(device)
            target_view = target_view.to(device)
